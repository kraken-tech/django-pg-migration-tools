...
class TestMultiSchemaConstraints:
    ...
    def test_unique_constraint_creation_in_second_schema(self):
        ...
        This is the actual bug that was fixed - without schema-awareness,
        this would raise ConstraintAlreadyExists.
        """
        ...
        with connection.cursor() as cursor:
            for schema in ["tenant1", "tenant2"]:
                cursor.execute(f"SET search_path TO {schema}")
                # Use a schema-aware query to truly verify the constraint exists
                cursor.execute(
                    """
                    SELECT con.conname
    ...
    def test_check_constraint_creation_in_second_schema(self):
        ...
        # Verify both schemas have the constraint using a schema-aware query
        # (not the potentially buggy CHECK_CONSTRAINT_IS_VALID)
        with connection.cursor() as cursor:
            for schema in ["tenant1", "tenant2"]:
                cursor.execute(f"SET search_path TO {schema}")
                # Use a schema-aware query to truly verify the constraint exists
