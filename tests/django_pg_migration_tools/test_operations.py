...
class TestSaferRenameModelPart2:
    app_label = "example_app"
    ...
    def _run_part_1(
        ...
        part_1_operation = operations.SaferRenameModelPart1(
            old_name="IntModel",
            new_name="NewIntModel",
        )
        part_1_operation.state_forwards(self.app_label, to_state)
        with connection.schema_editor(atomic=True, collect_sql=False) as editor:
            part_1_operation.database_forwards(
                self.app_label, editor, from_state=from_state, to_state=to_state
            )
    ...
    def test_when_not_allowed_to_migrate_by_the_router(self):
        project_state = ProjectState()
        project_state.add_model(ModelState.from_model(IntModel))
        part_1_state = project_state.clone()
        self._run_part_1(from_state=project_state, to_state=part_1_state)
        new_state = part_1_state.clone()
        operation = operations.SaferRenameModelPart2(
            new_name="NewIntModel",
            old_table_name="example_app_intmodel",
        )
        ...
        with connection.schema_editor(atomic=True, collect_sql=False) as editor:
            with utils.CaptureQueriesContext(connection) as queries:
                operation.database_forwards(
                    self.app_label, editor, from_state=part_1_state, to_state=new_state
                )
        ...
        with connection.schema_editor(atomic=True, collect_sql=False) as editor:
            with utils.CaptureQueriesContext(connection) as queries:
                operation.database_backwards(
                    self.app_label, editor, from_state=new_state, to_state=part_1_state
                )
    ...
    def test_basic_operation(self):
        ...
        assert operation.describe() == (
            "Using django_pg_migration_tools SaferRenameModelPart2 operation."
        )
        name, args, kwargs = operation.deconstruct()
        assert name == "SaferRenameModelPart2"
        assert args == ()
        assert kwargs == {
            "new_name": "NewIntModel",
            "old_table_name": "example_app_intmodel",
        ...
        assert len(queries) == 1
        assert queries[0]["sql"] == 'DROP VIEW IF EXISTS "example_app_intmodel";'

        # Moving forward again tries to drop the view again, but it is okay
        # since it will only drop IF EXISTS
        with connection.schema_editor(atomic=True, collect_sql=False) as editor:
            with utils.CaptureQueriesContext(connection) as second_forward_queries:
                operation.database_forwards(
                    self.app_label, editor, from_state=part_1_state, to_state=new_state
        ...
        assert len(second_forward_queries) == 1
        assert (
            second_forward_queries[0]["sql"]
            == 'DROP VIEW IF EXISTS "example_app_intmodel";'
        )
